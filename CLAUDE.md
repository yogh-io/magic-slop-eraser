...
- **Drafter** is an agentic coding tool (Claude Code, Codex, opencode, etc.) carrying the slopmop skill, your own scripts, or our hosted reviewer. Whatever speaks the API qualifies. The drafter does two jobs: in the analysis phase it dispatches **passes** (subagents) over the source - a voice memo first, then per-rung detection passes - and merges their findings into a flag batch posted via `POST /docs/:id/flags`. In the steering loop it dequeues author directives, drafts candidates, posts resolutions, advances state. The drafter does not read the prose during analysis - the passes do. Discipline: reading and merging are different jobs.
